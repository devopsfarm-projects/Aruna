...
export default function EditBlock() {
  ...
  const removeMeasure = (gIdx: number, bIdx: number, mIdx: number) => {
    if (!newBlock) return
    const updatedBlock = { ...newBlock }
    updatedBlock.group[gIdx].block[bIdx].addmeasures = updatedBlock.group[gIdx].block[
      bIdx
    ].addmeasures.filter((_, idx) => idx !== mIdx)
    setNewBlock(updatedBlock)
  }
        ...
        <form onSubmit={handleSubmit} className="bg-white dark:bg-gray-800 -2xl p-8 shadow-md">
          ...
          <div className="space-y-4">
            ...
            {currentBlock?.group?.map((group, gIdx) => (
              <div
                key={gIdx}
                className="border p-4 space-y-4 bg-gray-50 dark:bg-gray-800 rounded-md relative"
              >
                {/* Group Inputs */}
                {[
                  ...
                  <div key={name}>
                    ...
                    <input
                      ...
                      onChange={(e) => {
                        ...
                        const groupTotalArea =
                          group.block?.reduce((sum, block) => {
                            return (
                              sum +
                              block.addmeasures.reduce((areaSum, measure) => {
                        ...
                        const truck =
                          name === 'g_truck_cost'
                            ...
                            : parseFloat(group.g_truck_cost) || 0
                        const hydra =
                          name === 'g_hydra_cost'
                            ? parseFloat(e.target.value)
                            : parseFloat(group.g_hydra_cost) || 0
                        ...
                        group.block?.forEach((block, bIdx) => {
                          const blockArea = block.addmeasures.reduce(
                            (sum, measure) => sum + (parseFloat(measure.block_area) || 0),
                            0,
                          )
                          const blockCost = blockArea * (truck + hydra + todi)
                          handleNestedChange(
                            { target: { name: 'block_cost', value: blockCost.toFixed(2) } } as any,
                            'block_cost',
                            gIdx,
                            bIdx,
                            0,
                        ...
                        const totalBlockArea =
                          currentBlock?.group?.reduce((sum, g) => {
                            return (
                              sum +
                              (g.block?.reduce((areaSum, block) => {
                                return (
                                  areaSum +
                                  block.addmeasures.reduce(
                                    (measureSum, measure) =>
                                      measureSum + (parseFloat(measure.block_area) || 0),
                                    0,
                                  )
                                )
                              }, 0) || 0)
                ...
                {group.block.map((block, bIdx) => (
                  <div
                    key={bIdx}
                    className="ml-4 mt-2 border p-3 bg-white dark:bg-gray-800 rounded-md relative"
                  >
                    <div className="flex justify-between items-center mb-4">
                      <button
                        type="button"
                    ...
                    {block.addmeasures.map((m, mIdx) => (
                      <div
                        key={mIdx}
                        className="mt-4 space-y-2 border p-3 bg-gray-100 dark:bg-gray-700 rounded-md relative"
                      >
                        {[
                          ...
                          <div key={name}>
                            ...
                            <input
                              ...
                              onChange={(e) => {
                                ...
                                const l =
                                  name === 'l' ? parseFloat(e.target.value) : parseFloat(m.l) || 0
                                const b =
                                  name === 'b' ? parseFloat(e.target.value) : parseFloat(m.b) || 0
                                const h =
                                  name === 'h' ? parseFloat(e.target.value) : parseFloat(m.h) || 0
                                const area = l * b * h
                                handleNestedChange(
                                  { target: { name: 'block_area', value: area.toFixed(2) } } as any,
                                  'block_area',
                                  gIdx,
                                  bIdx,
                                  mIdx,
                                ...
                                group.block?.forEach((block, blockIdx) => {
                                  ...
                                  handleNestedChange(
                                    {
                                      target: { name: 'block_cost', value: blockCost.toFixed(2) },
                                    } as any,
                                    'block_cost',
                                    gIdx,
                                    blockIdx,
            ...
            <input
              ...
              value={
                newBlock?.total_block_area
                  ? Number(newBlock?.total_block_area).toLocaleString('en-IN')
                  : ''
              }
              disabled
            ...
            <input
              ...
              value={
                newBlock?.total_block_cost
                  ? Number(newBlock?.total_block_cost).toLocaleString('en-IN')
                  : ''
              }
              disabled
              className="w-full px-4 py-3 border border-gray-300 dark:border-gray-600 bg-gray-200 dark:bg-gray-600 text-gray-900 dark:text-white focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500"
