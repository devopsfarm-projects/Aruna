...
export default function EditBlock() {
  ...
  const handleDownloadPDF = async () => {
    ...
    try {
      ...
      pdf.setFontSize(24)
      pdf.text('Gala Block Report', 105, 30, { align: 'center' })
      pdf.setFontSize(12)
      pdf.setFont('helvetica', 'normal')
      pdf.text(`Gala Type: ${block.GalaType || 'N/A'}`, 105, 45, { align: 'center' })
      ...
      const filename = `TodiRaskat-${block.GalaType || 'Block'}-${new Date().toISOString().split('T')[0]}.pdf`
      pdf.setProperties({
        title: `Todi Raskat Block Report - ${block.GalaType || 'Block'}`,
      ...
      <div ref={formRef}>
        <div className="pdf-page bg-white p-6 mb-6 border border-gray-200">
          <section className="grid grid-cols-6 gap-4">
            <Info label="Gala Type" value={block.GalaType} />
            ...
            <Info label="Avg. Breadth (m)" value={block.total_b} />
            <Info label="Height (m)" value={block.h} />
            <Info label="Total Groups" value={block.group.length} />
            <Info label="Total Measures" value={block.group.reduce((total: any, group: { block: any[] }) => total + group.block.reduce((bT: any, b: any) => bT + b.addmeasures.length, 0), 0)} />
            <Info label="Total Blocks" value={block.group.reduce((total: any, group: { block: any[] }) => total + group.block.length, 0)} />
            <Info label="Total Gala Cost (₹)" value={Number(block.total_gala_cost).toLocaleString('en-IN')} />
            <Info label="Total Gala Area (m³)" value={Number(block.total_gala_area).toLocaleString('en-IN')} />
