...
export default function EditBlock() {
  ...
  useEffect(() => {
    const fetchBlock = async () => {
      if (!id) return
      try {
        const res = await axios.get(`/api/todi/${id}`)
  ...
  const handleDownloadPDF = async () => {
    ...
    try {
      ...
      const filename = `Todi-${block.BlockType || 'Report'}-${currentDate}.pdf`
      pdf.save(filename)
    ...
    <div className="mb-8 p-4 border border-gray-200 rounded-lg">
      ...
      <div className="grid grid-cols-3 gap-4">
        ...
        <div className="border p-3 rounded">
          <p className="text-sm text-gray-500">Total Block Area (m³)</p>
        ...
        {/* Total Block Cost */}
        <div className="border p-3 rounded">
          <p className="text-sm text-gray-500">Total Block Cost (₹)</p>
          <p className="text-xl font-bold">
