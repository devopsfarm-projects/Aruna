  ...
  hooks: {
    afterChange: [
        async ({ doc, req }) => {
          ...
          const totalPaid = allTransactions.docs.reduce((sum, txn) => sum + (txn.amountPaid || 0), 0);
      
          // Fetch the product to get final_total
          ...
          const partyRemaining = finalTotal - totalPaid;
      
          // Update product with new remaining payment
